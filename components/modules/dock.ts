...
import { togglePlayer, isPlayerOpen } from "./player.ts"
import { makePlane, strokePath, tiltText, fillQuad } from "./proj.ts"
...
const ICONF = "Symbols Nerd Font", TITLE = "Chakra Petch"
...
const VERT_TILES = [
 ...
 { key: "brt", icon: "", label: "", sc: "U", state: () => Promise.resolve(true) },
 { key: "mic", icon: "", label: "", sc: "M", state: () => sh("wpctl get-volume @DEFAULT_AUDIO_SOURCE@").then(o => !/MUTED/.test(o)) },
...
const VLAYOUT: { k: string; x: number; y: number; w: number; h: number }[] = [
 ...
 { k: "mic", x: BADGE + BGAP, y: 2 * (VSH + VG), w: VSW, h: VSH },
...
const horizSlotFrame = (x: number, y: number, w: number, h: number): [number, number][] => {
 const cutTL = Math.max(3, Math.min(w, h) * 0.14)  // top-left: 
 const cutTR = Math.max(5, Math.min(w, h) * 0.20)  // top-right 
 const cutBR = Math.max(2, Math.min(w, h) * 0.08)  // bottom-right
 const cutBL = Math.max(5, Math.min(w, h) * 0.20)  // bottom-left 
 return [
 [x + 1 + cutTL, y + 1],             // top-left bevel
 [x + 1, y + 1 + cutTL],
 [x + 1, y + h - 1 - cutBL],         // straight down the left edge
 [x + 1 + cutBL, y + h - 1],         // bottom-left bevel
 [x + w - 1 - cutBR, y + h - 1],     // straight across the bottom
 [x + w - 1, y + h - 1 - cutBR],     // bottom-right bevel
 [x + w - 1, y + 1 + cutTR],         // straight up the right edge
 [x + w - 1 - cutTR, y + 1],         // top-right bevel
 [x + 1 + cutTL, y + 1],             // close it back to the start
 ]}
...
const VertDock = () => {
 ...
 const area = DrawingArea({}); area.set_size_request(vsp.width + 10, vsp.height + 20)
 ...
 evt.connect("button-press-event", (_w, e) => { let x = 0, y = 0; try { const c = e.get_coords?.(); if (c) { x = c[1]; y = c[2] } } catch {} const k = hitSlot(x, y); if (k) { if (k === "music") togglePlayer(); else toggleModal(k); openRefresh() } return false })
 ...
 openRefresh = () => { let ch = false; for (const s of VLAYOUT) { const o = s.k === "music" ? isPlayerOpen() : isModalOpen(s.k); if (o !== open[s.k]) { open[s.k] = o; ch = true } } if (ch) area.queue_draw() }
 ...
 else { let busy = false; for (let i = 0; i < eqBars.length; i++) { if (eqBars[i] > 0.14) { eqBars[i] += (0.12 - eqBars[i]) * 0.3; busy = true } } if (busy) area.queue_draw() }
 })
...
const HorizDock = () => {
 ...
 const isz = s.h * 0.42 // icon size (fraction of tile height) -- a touch bigger than the vertical ones
  tiltText(ctx, hsp, s.x + s.w / 2 + 0.1, s.y + s.h * 0.60, t.icon, ICONF, isz, edge, Math.min(1, (on[s.k] ? 1 : 0.78) + hov * 0.22), { align: "c",extraRotate: 0.05, glow: (on[s.k] ? 0.4 : 0) + hov * 0.5 })
 ...
 evt.connect("motion-notify-event", (_w, e) => { let x = 0, y = 0; try { const c = e.get_coords?.(); if (c) { x = c[1]; y = c[2] } } catch {} hovered = hitSlot(x, y); return false })
 evt.connect("leave-notify-event", () => { hovered = null; return false })
 openRefresh = () => { let ch = false; for (const s of HLAYOUT) { const o = isModalOpen(s.k); if (o !== open[s.k]) { open[s.k] = o; ch = true } } if (ch) area.queue_draw() }
