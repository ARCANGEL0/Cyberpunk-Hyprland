...
// the little cyan badge on the left shows CPU temperature if it can find a thermal sensor,
// and quietly falls back to computer uptime on machines that don't expose one.
...
const plane = makePlane({ w: W, h: H, yaw: 3, pitch: 2, roll: 4, focal: 1300, dist: 1300, pad: 22 })
...
export const Monitors = () => {
    ...
    const hitTest = (x: number, y: number): string | null => {
        ...
        const [bx_l, bx_t] = plane.project(4, 14)
        const [bx_r, bx_b] = plane.project(48, 54)
    ...
    area.connect("draw", (_w: any, ctx: any) => {
        ...
        // ── the badge: a box with the bottom-left corner chamfered, holding the temp/uptime number ──
        const bx = 4, by = 14, S = 40
        ...
        tiltText(ctx, plane, bx + 20, by + 24, num, TITLE, 15, CYAN, 0.95, { align: "c", bold: true, glow: 0.4 })
        ...
        // ── CPU: the big red bar (the "health bar"). its bottom-right corner is beveled, 
        // ── and the fill has to follow that bevel once it gets near full -- hence the two-case fillPt
        {
        ...
        // ── RAM: a row of little angular ticks instead of a solid bar; like the game, its like some "flash chips"
        {
        ...
        // ── BATTERY: a parallelogram rail sitting on its own below the meters, colour from batColor ──
        {
        ...
        // ── the numeric text readouts down the right side (disk used/total, cpu%, ram%, battery%) ──
                tiltText(ctx, plane, W, 11, `${diskUsedG}/${diskTotG}G`, MONO, 8, CYAN, 0.85, { align: "r", bold: true })
        tiltText(ctx, plane, W, 36, cpu.percent.get(), TITLE, 24, LIGHTRED, 1, { align: "r", bold: true, glow: 0.18 })
        tiltText(ctx, plane, W, 56, ram.percent.get(), TITLE, 11, CYAN, 0.95, { align: "r", bold: true })
    ...
    const t = interval(110, () => {
        ...
        const sig = `${cpu.percent.get()}|${ram.percent.get()}|${diskUsedG}|${Math.round(batteryV * 100)}`
