...
warn()  { printf "  ${YEL}⚠${R} %s\n" "$1"; }
err()   { printf "  ${RED}✗${R} %s\n" "$1"; }
ask()   { local a; read -rp "$(printf "${YEL}%s ${R}${DIM}(y/N)${R} " "$1")" a </dev/tty; [[ "$a" =~ ^[Yy]$ ]]; }
banner() {
  printf "${RED}${B}"
  cat <<'EOF'
   █▀▀ █▄█ █▄▄ █▀▀ █▀█ █▀█ █░█ █▄░█ █▄▀   ▀█ █▀█ ▀▀█ ▀▀█
...
REPO=(
  gjs awww grim wf-recorder wl-clipboard networkmanager bluez-utils
  wireplumber playerctl brightnessctl power-profiles-daemon upower
  hypridle socat jq rofi libnotify sassc kitty kvantum kvantum-qt5 wget fuse2 sqlite3
...
AUR=(
  ...
  aylurs-gtk-shell
  libastal-gjs-git libastal-notifd-git libastal-wireplumber-git
  pamtester
)
...
if [ -n "$HYP_RUN" ] && [ "$(printf '%s\n%s\n' "$HYP_MIN" "$HYP_RUN" | sort -V | head -1)" != "$HYP_MIN" ]; then
  ...
  if ask "Update Hyprland now ($HYP_RUN → $HYP_MIN)?"; then
...
if [ ${#miss_repo[@]} -gt 0 ] || [ ${#miss_aur[@]} -gt 0 ]; then
  ...
  if [ ${#miss_repo[@]} -gt 0 ]; then
    ...
    if ask "Install missing repo deps?"; then
      if sudo pacman -S  lib32-libelf "${miss_repo[@]}"; then
  ...
  if [ ${#miss_aur[@]} -gt 0 ]; then
    ...
    elif ask "Install missing AUR deps via $(basename "$helper")?"; then
      if "$helper" -S  "${miss_aur[@]}"; then
...
# ── 3.5 · UI fonts (Chakra Petch + Rajdhani) ──────────────────────────
hdr "UI FONTS"
...
if [ -d "$FONTSRC" ] && compgen -G "$FONTSRC/*.ttf" >/dev/null; then
  ...
  fc-cache -f "$FONTDST" >/dev/null 2>&1 && ok "font cache refreshed (Chakra Petch · Rajdhani)" || warn "fc-cache not run (install fontconfig)"
else
  warn "bundled fonts missing at $FONTSRC |::| Chakra Petch/Rajdhani text will fall back to sans-serif."
fi
...
elif ask "Add the theme source to the TOP of hyprland.conf?"; then
  TMP="$(mktemp)"
  {
...
kb_scan() {
  ...
  while IFS= read -r ln; do
    ...
    if ask "  COMMENT THIS LINE IN $(basename "$f") AND SET THEME DEFAULT?"; then
      sed -i "${n}s|^|#|" "$f" && ok "commented $(basename "$f"):$n"
    else
...
if [ "${NEED_RESTART:-0}" = 1 ]; then
  ...
  if ask "  Restart Hyprland now?"; then
