...
class CHyprBar : public IHyprWindowDecoration {
  ...
  private:
    ...
    // cyber: whole-bar cairo texture (beveled shape + neon glow + frame + wireframe buttons)
    SP<Render::ITexture>       m_pBarTex;
    Vector2D                   m_vBarTexSize;
    std::string                m_szBarTexTitle;
    bool                       m_bBarTexFocus = false;
    int                        m_iHoveredButton = -1;   // which button the cursor is over (-1 none)
    int                        m_iBarTexHover   = -2;   // hover state baked into the current texture
    ...
    void                       renderBarCairo(const Vector2D& bufferSize, const float scale, const bool focus, const int hovered);
